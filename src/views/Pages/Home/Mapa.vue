...
const addButtonStyle = computed(() => ({
  bottom: 16 + ui.footerPaddingBottom + "px",
}));
...
function makeBodyTransparent() {
  ...
  prevStyles.ionBgVar = getComputedStyle(document.documentElement).getPropertyValue(
...
async function createGoogleMap(): Promise<void> {
  if (!GMAPS_KEY) {
    showToast("Falta la API Key de Google Maps.");
    throw new Error("GMAPS_KEY missing");
  }

  if (!mapEl.value) {
    showToast("Contenedor del mapa no está listo.");
  ...
  // Si ya había un mapa (por navegación rara), destrúyelo primero
  if (map) {
    try {
      if (markerIds.length) {
  ...
  // Cámara / ubicación actual (si falla, no truena)
  try {
  ...
  await map.setOnMarkerClickListener(async (event) => {
    ...
    try {
      await map?.setCamera({
        coordinate: { lat: clicked.lat, lng: clicked.lng },
        zoom: 16,
        animate: true,
      });
    } catch {}
...
function clampToRadius(center: LatLng, target: LatLng, maxKm: number): LatLng {
  ...
  if (!dist || dist <= maxKm) return target;
...
async function attachCameraLimits(center: LatLng): Promise<void> {
  ...
  await map?.setOnCameraIdleListener(async (camera) => {
    ...
    let target: LatLng = { lat: camera.latitude, lng: camera.longitude };
...
async function ofertasMap(id: string, marca: string): Promise<void> {
  ...
  try {
    ...
    if (resp.status === "ok" && (allPromos.value.length > 0 || espPromos.value.length > 0)) {
      openOfertas.value = true;
    } else {
      showToast("No hay ofertas agregadas.");
    }
  } catch {
...
<template>
  <ion-page class="mapa">
      ...
      >
        ...
        <ion-content :fullscreen="true">
          <div class="w-full h-full flex flex-col items-start gap-2 bg-[#f5f5f5] dark:bg-[#121212] p-5">
            <TransitionGroup name="list-fade" tag="div" appear class="w-full flex flex-col gap-5">
              <img
                  v-for="a in allPromos"
                  :key="`all-${a.promo_id}`"
                  :src="urlPromos(a.img_promo)"
                  :alt="a.img_promo"
                  loading="lazy"
                  class="shadow-md rounded-4xl"
              />
              <img
                  v-for="e in espPromos"
                  :key="`esp-${e.promo_id}`"
                  :src="urlPromos(e.img_promo)"
                  :alt="e.img_promo"
                  loading="lazy"
                  class="shadow-md rounded-4xl"
              />
    ...
    <div
        v-if="espPromos.length > 0 || allPromos.length > 0"
        class="fixed inset-x-0 w-full flex justify-center pointer-events-none"
...
/* ✅ importante: relative para que el absolute del mapa funcione bien */
ion-content.mapa {
  --background: transparent !important;
  position: relative;
}
